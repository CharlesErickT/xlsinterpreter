function multiUploader(config){
		...
		if(this.items.length > 0){
			...
			var uId = "";
 			for(var i = 0; i<this.items.length; i++){
				...
				var sampleIcon = '<img src="images/excel.png" />';
				var errorClass = "";
	...
	multiUploader.prototype._dropFiles = function(e){
		if (fileLimit == false){
			...
			var htmlError = '<h4 class="limitError" style="color:red">You can only upload 1 file at a time !</h4>';
			e.stopPropagation(); e.preventDefault();
			$(".title").append(htmlError);
		}
	}
				...
				success:function(rponse){
					...
					$.each(obj,function(k,fle){
						if($(fle).attr("rel") == rponse){
							...
							fileLimit = false;
							if ($(".limitError")[0]){
								$(".limitError").slideUp("normal", function(){ $(".limitError").remove(); });
							}
						}
					});
					if (f+1 < file.length) {
